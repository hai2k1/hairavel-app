...
trait FileManage
{
    ...
    private function getFile($dirId, $query = '', $filter = 'all'): array
    {
        ...
        if ($dirId) {
            $data = \Hairavel\Core\Model\File::where('has_type', $this->getHasType())->where('dir_id', $dirId);
            if ($query) {
    ...
    {
        return \Hairavel\Core\Model\FileDir::where('has_type', $this->getHasType())->get()->toArray();
    }
    ...
    {
        if (empty($name)) {
            trigger_error('Please enter the directory name');
        }
        $file = new \Hairavel\Core\Model\FileDir;
        $file->name = $name;
    ...
    {
        if (empty($id)) {
            trigger_error('Please select a directory');
        }

        $files = \Hairavel\Core\Model\File::where('has_type', $this->getHasType())->where('dir_id', $id)->get([
            'driver', 'path'
        ]);
        $files->map(function ($vo) {
            Storage::disk($vo->driver)->delete($vo->path);
        });
        \Hairavel\Core\Model\File::where('file_id', $id)->delete();
        \Hairavel\Core\Model\FileDir::where('dir_id', $id)->delete();
        return [];
    }
    ...
    private function deleteFile($ids): array
    {
        ...
        if (empty($ids)) {
            trigger_error('Please choose to delete the file');
        }
        $files = \Hairavel\Core\Model\File::where('has_type', $this->getHasType())->whereIn('dir_id', $ids)->get([
            'driver', 'path'
        ]);
        $files->map(function ($vo) {
            Storage::disk($vo->driver)->delete($vo->path);
        });
        \Hairavel\Core\Model\File::whereIn('file_id', $ids)->delete();
        return [];
    }
