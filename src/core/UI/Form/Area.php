...
class Area extends Element implements Component
{
    ...
    {
        $info = \Hairavel\Core\Model\Area::where(['code' => $data])->first();

        $code = $info->parent_code;
        if ($info->level > 3) {
            $region = \Hairavel\Core\Model\Area::where(['code' => $code])->first();
            $code = $region['parent_code'];
        }
        if ($info->level > 2) {
            $city = \Hairavel\Core\Model\Area::where(['code' => $code])->first();
            $code = $city['parent_code'];
        }
        if ($info->level > 1) {
            $province = \Hairavel\Core\Model\Area::where(['code' => $code])->first();
        }
        $data = [];
        if ($region) {
